...
That comparison uses **one clock, on purpose**. An earlier cut compared `readAtMs - mtimeMs`, subtracting a filesystem-reported mtime from the daemon's own `Date.now()` — two clocks, and the coarse-granularity case motivating the whole mechanism (a network mount) is exactly where they are also *skewed*. With a lagging server clock a just-written file computed as long-settled and was skipped on the next poll, silently reopening the gap. Elapsed time since our own read, measured only on our own clock, has no such failure mode. `readAtMs` is stamped only when bytes were read or a held fragment was parsed, never on a hash check that matched — otherwise the window restarts on every check and never settles. An actively-written transcript is therefore hashed for roughly three 667ms polls after its last write and then goes quiet; an idle one still costs one `stat` for itself (two for a registered `claude -p` child) and one per transcript its last attribution folded. A transcript whose turn spawns a `claude -p` is re-attributed every poll until that turn's discovery window closes, whenever its own last write was. Its bytes are read only when they grew or the file rotated. A matching hash appends nothing and does not move the offset. `tests/wtft-420-subagent-call-site.test.ts` allows the whole-file parse's call (in `parseSessionFileCounted`), the session tagger's adapter forwarding it (`fsWorld.attribute`) and the tagger's one port call (`world.attribute` in `syncSubagentTranscript`) on every retained fold-capable turn, and fails when any other production call site appears.
...
**That is a strong default, and the violation is COUNTED.** `getSessionSummary` in `extensions/lib/session-selector.ts` reads through `classifiedInteractionsFromContent`, the same current-generation, id-collapsed read the report uses (wtft#270 S1); `tests/wtft-270-session-summary-dedup.test.ts` pins its figures against the canonical path. **A new reader that bypasses `readClassifiedTagFile` and does not add a similar pinning test reintroduces the raw-sum overcount.** `tests/wtft-tag-reader-collapse-guard.test.ts` enumerates every source file that both resolves a tag path (`getTagPath` / `TAG_SUFFIX` / a literal `.wtft-tag.`) and reads tag lines (`JSON.parse`, `tagRecords`, `parseTagLine` or a canonical reader), and fails unless each one either routes through `readClassifiedTagFile`, `readTagFileWithVerdict`, `seedClassifiedTagFile`, `classifiedInteractionsFromContent` or `dedupeClassifiedById`, or appears in an allowlist carrying its reason. The allowlist is checked for rot in the same run, so a stale exemption cannot quietly accumulate.
...
Route new readers through `readClassifiedTagFile` or `classifiedInteractionsFromContent`; a reader that cannot pins its copy with a test and carries an allowlist entry, so the exception is visible instead of merely absent. Every reader decides a line's kind through `parseTagLine` or `tagRecords` in `extensions/lib/tag-log.ts`, never by substring (wtft#270 S1).
...
**Already-finished `claude -p` sub-agent transcripts are reached by the resume, not by discovery** (PR review, then #259). Those are discovered from the other direction: a bash turn in the PARENT transcript is matched to a transcript path, and `pendingClaudeCommands` is fed from lines `parseNewLines` has just read (`extensions/lib/session-tagger.ts`, the `hasClaudeCommand(interaction)` push) and, on a restart, from the tag's open `spawnPending` records. On a restart where the tag file yields a `_meta` offset, `initClassified` resumes `lastSize` at that offset — the end of the last whole line consumed, so a record still being written when the marker was stamped is re-read whole and no other parent line is; instead `resumeTagger` re-registers each `claude -p` child the tag's generation records name that is not left to another holder or to discovery, listing `~/.claude/projects` for each id, and every child a settled lookup listed that no generation record already names and that is still on disk, and reads each again from its start as a new generation (`docs/spec-259-daemon-correctness.md` § Resume). A lookup settled with no `children` list, which only a pre-#259 daemon wrote, is the case the resume cannot see; that was **#456**.
...
`syncSubagentTranscript`'s stat, read, parse, attribution and serialize handlers, the discovery and reseed failures in `scanChildren`, its stat of a registered `claude -p` child and of a transcript a sliced pass read (any error but a missing file or directory), and a failed read of the session transcript — so a poll that could not read what it was asked to does not claim to have
...
#270 S3 a missing file or directory is gone, not a failure: the turn it held is written
...
withholds FUTURE stamps; a marker stamped BEFORE a child transcript's failure began is not
retracted (a failed read of the session transcript itself does retract it, with an
`unswept` record), so while the parent stays quiet the tag still reads settled with the
unreadable child's cost missing. The remaining gap is only the ordinary one: a failure
