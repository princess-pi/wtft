...
| `claude -p` bash sub-agents (#138) | `discoverClaudeSubAgentSessionFiles(cwd, ts)`, re-run every poll until a file matches and the bash turn's discovery window has closed; each matched PATH is then kept | `syncSubagentTranscript(file)` — the same function | `discoveredClaudeFiles: Set<path>` for the registry, plus the same `discoveredSubagentFiles` entry as any other transcript |
