...
the hand-off read, as `parseHandOff` took it over, and `--restart` stopping, waiting for and
respawning only a live daemon; the reconcile also reworded `--restart`'s stdout lines and `--help` text,
