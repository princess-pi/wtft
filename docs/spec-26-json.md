...
| `spawned.total` | object | The sum over RESOLVED descendants. Where a descendant's parse folded in a session already in `total` or already counted in this tree, that session's share is subtracted from the descendant's total. Unattributed and depth-capped edges are not in it. |
