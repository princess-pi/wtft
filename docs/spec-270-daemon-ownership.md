...
state, not behaviour: the process-level suites pass unchanged, and the one check it adds is the
scan-continuation flag surviving a move (#267 F), which is now a field of the moved record.
