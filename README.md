...
- **1** — error: no session found or selected, an invalid path, a daemon that
  ...
  stop, a session another daemon claimed, a lease or tag file that could not be deleted, a daemon
  that could not be started, or a harness that did not take the session up within
