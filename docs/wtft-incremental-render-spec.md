...
- **Claude Code puts a Task subagent in `<session-dir>/<session-id>/subagents/`, never in the parent file.** `discoverSubagentSessionFiles` walks that directory and every directory under it (the `workflows/wf_<id>/` children are one level down), and lists only files named `agent-*.jsonl`. So `agent-<id>.meta.json`, the sidecar #137 reads, is never mistaken for a transcript. The parent transcript holds no sidechain record at all: 0 `"isSidechain":true` lines in a 478-record session with two subagents, measured on Claude Code 2.1.281 on 2026-09-23. So a collector that stops at `projects/*/*.jsonl` misses all subagent spend, and still reports a plausible number.
- **Pi puts a subagent beside its parent**, as a sibling `.jsonl` whose header names the parent in `parentSession`.
- **Session discovery skips a directory named `subagents` in both harnesses** (`SKIP_DIRS` in `extensions/lib/harness/claude-code/discovery.ts` and `extensions/lib/harness/pi/discovery.ts`). So a subagent transcript is never listed as a session of its own, and its cost is never counted beside the parent's. Subagent discovery is the only reader of that directory.
...
`tests/wtft-15-read-path-doc-claims.test.ts` quotes the sentences in this section and in the two sections on `attributeClaudeSubAgentCosts` and `deduplicateInteractions` below, and drives the code each one describes.
