...
| M7b | **the HARNESS's names**, against the NEWEST real `.meta.json` on this host (picked by mtime over a recursive walk — an arbitrary pick kept selecting a two-day-old file while the corpus ran to today, so a rename shipped now would leave 400+ stale files keeping it green). Host-gated: it SKIPS VISIBLY where there is no `~/.claude`, so CI does not check it. It also fails when that file carries a key no file in the committed corpus has, which is the signal to refresh the corpus |
| M7c | **the harness's names as captured**, against the committed corpus of real files in `tests/fixtures/meta-corpus/`. Runs on every host, CI included, and sees a snapshot, not today's harness: both required names, the near-universal pair except on a workflow child, and the reader accepting each file (`docs/spec-194-p9-housekeeping.md` § H2) |
