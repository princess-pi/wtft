...
| `docs/spec-95-daemon-lifecycle.md`, `CONTEXT.md`, `README.md` | one daemon per session, and a version bump replaces it | a harness root shares one process; a live one is left running and the session lease is pointed at it when `/proc/<pid>/cmdline` names the log parser daemon | shared process and idle drop: `tests/wtft-205-one-daemon-per-harness.test.ts`. The version-bump sentence is `reconciled-against-untested` | Fixed |
| `docs/spec-114-14-generation-records.md` | a generation with no lines still writes its record | a first read of a nonempty file that produced no interactions did not | `tests/wtft-97-subagent-offset.test.ts` (junk transcript) | Fixed in the daemon |
