...
| Key | Type | Meaning |
|---|---|---|
| `schema` | string | `"wtft/session@1"`. Bumped when any key below changes **shape**. Adding a new top-level key does not: a consumer reading known keys is unaffected, and one reading unknown keys was already reading a document it did not fully understand. `subagents[]` was added under `@1` on that rule (#137). A consumer that needs to know whether a given wtft can report subagents should test for the key, not the schema string. |
...
| `subagents[]` | array \| **absent** | #137. One row per built-in (Task) subagent transcript discovered for this session. **Absent, not `[]`, on the `pending-session` arm** — the session file does not exist, so discovery never ran, and an empty array there would read as "spawned nothing" rather than "nobody looked". Same rule as `uncounted`, opposite spelling, because this one has a meaningful absent state and that one does not. |
| `subagents[].transcript` | string | Always present. The child's `.jsonl`, which is where its cost comes from. |
| `subagents[].meta` | object \| null | The `.meta.json` the harness writes beside the transcript: `agentType`, `description`, `toolUseId`, `spawnDepth`, and optionally `model`, `parentAgentId`, `isFork`. **`null` is a GAP, not a missing subagent** — the transcript is listed and counted either way, it simply has no label. Pi children, shell-spawned children and any harness release that stops writing the file all land here. `meta.model` is itself optional: 419 of 439 files on this host carry one. |
