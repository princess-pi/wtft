...
- **A folded child is measured once.** A selected transcript that another selected session
  folds is dropped from the measured list, with a stderr note, so it is counted only inside
  its parent. Otherwise a newly found fold of an already-selected session would count that
  child twice and still read as an explained rise.
...
| Macroscope, on Draft pushes | 2 Medium threads: a child both selected and newly folded counted twice yet "explained"; the seam probe blocking Pi-only runs | the selection, `honoursProjectsSeam` | ✅ G18 | First fixed: a folded child is measured only inside its parent (a different remedy from the thread's, which would have failed a real discovery). Second declined: a Pi transcript's parse runs `attributeClaudeSubAgentCosts`, which discovers `claude -p` children under the Claude Code projects root, so the seam matters for a Pi-only selection too. **Measured for duppypro/princess-pi-tools#1027:** after a PR has been Ready once, Macroscope reviews every push even while it is back in Draft (4 rounds on 2ab6c5b, 05faaf1, e084bb6, bae22db). Returning to Draft does not make pushes free; holding the pushes does |
