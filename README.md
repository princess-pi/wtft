...
for a tag of this version (an older-version tag is taken over), and a harness start takes a
per-session holder's lease and retries on another harness's, so the daemon revives after an
idle timeout when the previous process exited. A session under the Claude
