...
| **1** | Error: no session found or selected, an invalid path, a daemon that could not be spawned or that died before producing data, a refused flag (`-p`), a read error other than a missing path in a harness's session tree while `--json` lists unrecorded spawns (#212), or an unhandled exception. The reason is on stderr. | nothing |
