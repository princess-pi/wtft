...
| `spawned.descendantUntagged[]` | array | `{child, untaggedInteractions, untaggedCostUsd}`, one per counted descendant (a `resolved` edge) whose own parse holds untagged turns — no model id, as `untaggedInteractions` counts them for this session (#180, Amendment 7). Their cost is **not** in that edge's `total`, and it is added to neither `spawned.total` nor `tree`, as this session's own untagged cost is not in `total.costUsd` — except for the overlap below. `untaggedCostUsd` is often `0`; the entry is there because the turns exist. It is computed from the descendant's own transcript and the `claude -p` children it folds, not its Task subagents (#230), so it can differ from that descendant's own `wtft --json` `total.untaggedCostUsd`. An untagged turn that spawned a `claude -p` child carries that child's share; such a child is not marked folded, so if the ledger also records it, its cost is in `spawned.total` as well. `[]` when no counted descendant has any. |
