...
| `s` | string | optional | Source: set on a line the daemon wrote from a child transcript, absent on the tag's own session's lines. The first 16 hex digits of the SHA-1 of the child transcript's path — relative to the session directory when it lies under it, absolute when it does not — as the paths stood at the daemon's first read of that child; a later move of the session does not change a child's `s` (#263). A later `_gen` record for the same `s` supersedes the line (§2e) |
