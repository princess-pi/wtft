...
| M7c | **the HARNESS's names**, against the committed corpus of real files in `tests/fixtures/meta-corpus/`. Runs on every host, CI included: both required names, the near-universal pair except on a workflow child, and the reader accepting each file (`docs/spec-194-p9-housekeeping.md` § H2) |
