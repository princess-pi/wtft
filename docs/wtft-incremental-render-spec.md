...
| `claude -p` bash sub-agents (#138) | `discoverClaudeSubAgentFilesForTurn(commands, ts, ownCwd)` — one `discoverClaudeSubAgentSessionFiles(cwd, ts)` per spawning segment, with the session's own cwd standing in for a spawn that no `cd` precedes and whose own segment runs `claude` itself, never for a launcher that only names it in a flag (#107) — re-run every poll; once a file matches, until the bash turn's discovery window has closed. A turn that has searched and found NOTHING has no time bound and is re-run for the daemon's life, and one with nothing to search waits out its window and is dropped (#107). Each matched PATH is then kept | `syncSubagentTranscript(file)` — the same function | `discoveredClaudeFiles: Set<path>` for the registry, plus the same `discoveredSubagentFiles` entry as any other transcript |

**Discovery is one-shot; reading is not.** A matched transcript stays matched — that match cannot un-happen — so `discoveredClaudeFiles` records each path, and discovery for that bash command stops once its window has closed (`docs/spec-114-14-generation-records.md`). Reading that path then happens on every poll, through the same `syncSubagentTranscript` the Task/agent path uses: re-parsed WHOLE on every change the gate sees — `size`, `mtimeMs` or inode, a folded transcript's stat, or an open spawn window — appending only lines not already on disk, except when the parse opens a new generation, which re-appends the whole parse behind a `_gen` record that supersedes what came before.
...
**The fix**: on every poll, `scanForSubAgents` (`bin/wtft-daemon.ts`, `scanForSubAgents`) calls `syncSubagentTranscript` for each known Task/agent sub-agent file, which stats it; if the gate fires, it re-parses the file WHOLE — `deduplicateInteractions(parseSessionFile(file, ancestors))` (`bin/wtft-daemon.ts`, the re-parse call in `syncSubagentTranscript`), the exact single-call shape both functions above assume, with the watched session as the ancestor so a child cannot fold it back in (#107) — and appends only the serialized lines not already on disk for that file (`bin/wtft-daemon.ts`, the append filter in `syncSubagentTranscript`). A quiet transcript costs one `stat`, plus one for each `claude -p` transcript its last parse folded, and nothing else (`bin/wtft-daemon.ts`, the change gate in `syncSubagentTranscript`). That gate fires on a `size`, `mtimeMs` or inode change, on a change to any folded transcript, and while a spawning turn's discovery window is open; a parse that drops a written line the reader cannot dedup opens a new generation (`docs/spec-114-14-generation-records.md`). A transcript another synced transcript folds is not synced on its own — the same discovery window returns both the child and the grandchild it folds — and if it was synced before that fold was seen, a generation record retires the lines it wrote (#107).
