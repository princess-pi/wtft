...
| **10** | `EXIT_SESSION_AMBIGUOUS` (#89): no interactive terminal, and either `-s <substring>` matched zero or several sessions, or no `-s` was given and the picker's own default-scoped population (this worktree, last 20 minutes) held zero or several sessions -- ZERO included, which replaces the OLD exit 1 "no session found" for this specific no-`-s`/no-TTY case. Every candidate is named on stderr. | nothing |
...
comparisons use, and the internal arithmetic itself is exact (both sides sum
the same per-interaction figures). `tests/wtft-119-untagged-cost.test.ts` is
that assertion.
